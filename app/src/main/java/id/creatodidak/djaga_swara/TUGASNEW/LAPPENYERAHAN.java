...
public class LAPPENYERAHAN extends AppCompatActivity {
    String IDTPS;
    DBHelper dbHelper;
    Endpoint endpoint;
    EditText etFakta;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lappenyerahan);
        ...
        formPamTps.setVisibility(View.GONE);
        ...
        String status = dbHelper.cekLapserah(IDTPS);
    ...
    private void saveDataToDB() {
        String FAKTA = etFakta.getText().toString();
        ...
        if (dbHelper.saveLapSerahTPS(IDTPS, FAKTA, ANALISA, PREDIKSI, REKOMENDASI, FOTO)) {
            tryUploadData();
        }
    }
    ...
    private void showPreview(String photoPath) {
        ...
        intent.putExtra("judul", "PENYERAHAN KOTAK SUARA TPS " + dbHelper.getRincianTpsById(IDTPS).getNomorTps() + " Desa" + sh.getString("DESA TPS", "TIDAK DIKETAHUI"));
        intent.putExtra("koordinat", SessionData.getNamaPangkat(LAPPENYERAHAN.this).toUpperCase());
    ...
    private void tryUploadData() {
        ...
        MLapSerahTPS data = dbHelper.getLapSerahTPS(IDTPS);
        ...
        Call<MResponseServer> call = endpoint.uploadLaporanSerahTPS(IDTPS, FAKTA, ANALISA, PREDIKSI, REKOMENDASI, imageParts);
        call.enqueue(new Callback<MResponseServer>() {
            ...
            public void onResponse(Call<MResponseServer> call, Response<MResponseServer> response) {
                if (response.body().isStatus() && response.body() != null && response.isSuccessful()) {
                    if (dbHelper.updStatusSerahTPS(IDTPS)) {
                        alerts.dismiss();
                        CDialog.up(
                                LAPPENYERAHAN.this,
                                "Informasi",
    ...
    private void showdata(boolean showbuttondraft)  {
        ...
        String[] FOTO = data.getDokumentasi().split(",");
        String pre = "Kepada: Yth. Kapolres Landak\nDari: Pers Pam TPS " + dbHelper.getRincianTpsById(IDTPS).getNomorTps() + " Desa " + TextHelper.capitalize(sh.getString("DESA TPS", "TIDAK DIKETAHUI")) + "\n\n" + WaktuLokal.deteksiWaktu() + " Komandan, izin melaporkan kegiatan Penyerahan Kotak Suara TPS " + dbHelper.getRincianTpsById(IDTPS).getNomorTps() + " Desa " + TextHelper.capitalize(sh.getString("DESA TPS", "TIDAK DIKETAHUI")) + " kepada PPK yang disaksikan/dikawal oleh " + TextHelper.capitalize(SessionData.getuserdata(this).getPangkat() + " " + SessionData.getuserdata(this).getNama()) + " sbb:\n";
        ...
        if (formPamTps.getVisibility() == View.VISIBLE) {
            CDialog.up(this,
                    ...
                    new CDialog.AlertDialogListener() {
                        @Override
                        public void onOpt1(AlertDialog alert) {
                            Intent intent = new Intent(LAPPENYERAHAN.this, JobSelector.class);
