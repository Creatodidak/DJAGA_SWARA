...
public class LAPPENGAWALAN extends AppCompatActivity {
    String IDTPS;
    DBHelper dbHelper;
    Endpoint endpoint;
    EditText etFakta;
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_lappengawalan);
    ...
    private void saveDataToDB() {
        ...
        if (dbHelper.saveLapWalTPS(IDTPS, FAKTA, ANALISA, PREDIKSI, REKOMENDASI, FOTO)) {
            tryUploadData();
        }
    }
    ...
    private void showPreview(String photoPath) {
        ...
        intent.putExtra("judul", "PENGAWALAN KOTAK SUARA TPS " + dbHelper.getRincianTpsById(IDTPS).getNomorTps() + " Desa" + sh.getString("DESA TPS", "TIDAK DIKETAHUI"));
        intent.putExtra("koordinat", SessionData.getNamaPangkat(LAPPENGAWALAN.this).toUpperCase());
    ...
    private void tryUploadData() {
        ...
        MLapWalTPS data = dbHelper.getLapWalTPS(IDTPS);
        ...
        Call<MResponseServer> call = endpoint.uploadLaporanWalTPS(IDTPS, FAKTA, ANALISA, PREDIKSI, REKOMENDASI, imageParts);
        call.enqueue(new Callback<MResponseServer>() {
            ...
            public void onResponse(Call<MResponseServer> call, Response<MResponseServer> response) {
                if (response.body().isStatus() && response.body() != null && response.isSuccessful()) {
                    if (dbHelper.updStatusWalTPS(IDTPS)) {
                        alerts.dismiss();
                        CDialog.up(
                                LAPPENGAWALAN.this,
                                "Informasi",
    ...
    private void showdata(boolean showbuttondraft)  {
        ...
        String[] FOTO = data.getDokumentasi().split(",");
        String pre = "Kepada: Yth. Kapolres Landak\nDari: Pers Pam TPS " + dbHelper.getRincianTpsById(IDTPS).getNomorTps() + " Desa " + TextHelper.capitalize(sh.getString("DESA TPS", "TIDAK DIKETAHUI")) + "\n\n" + WaktuLokal.deteksiWaktu() + " Komandan, izin melaporkan kegiatan Pengawalan Kotak Suara TPS " + dbHelper.getRincianTpsById(IDTPS).getNomorTps() + " Desa " + TextHelper.capitalize(sh.getString("DESA TPS", "TIDAK DIKETAHUI")) + " ke PPK oleh " + TextHelper.capitalize(SessionData.getuserdata(this).getPangkat() + " " + SessionData.getuserdata(this).getNama()) + " sbb:\n";
