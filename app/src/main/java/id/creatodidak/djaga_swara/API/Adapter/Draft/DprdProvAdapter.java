...
public class DprdProvAdapter extends RecyclerView.Adapter<DprdProvAdapter.ViewHolder> {
    ...
    public DprdProvAdapter(List<DPRDProv> dprdProvList, Activity activity) {
        this.dprdProvList = dprdProvList;
        mActivity = activity;
    }

    public void setData(List<DPRDProv> data) {
        dprdProvList.clear();
        if (data != null) {
            dprdProvList.addAll(data);
        }
    ...
    @NonNull
    @Override
    public DprdProvAdapter.ViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.draftsuara, parent, false);
        return new DprdProvAdapter.ViewHolder(view);
    }

    @Override
    public void onBindViewHolder(@NonNull DprdProvAdapter.ViewHolder holder, int position) {
        DPRDProv dprdprov = dprdProvList.get(position);
        databaseHelper = new DatabaseHelper(holder.itemView.getContext());
        TpsList tpsList =databaseHelper.getSprindetail(dprdprov.getId_tps());

        holder.judul.setText("Laporan Jumlah Suara Pemilihan DPRD Provinsi Nomor Urut "+dprdprov.getNomorurut()+" di TPS "+tpsList.getNomorTps()+" Desa "+tpsList.getNamaDes());
        holder.nourut.setText(dprdprov.getNomorurut());
        holder.suara.setText(dprdprov.getSuara());
        holder.nama.setText(dprdprov.getNama());
            ...
            public void onClick(View v) {
                ...
                builder.setPositiveButton("Ya, kirim...", new DialogInterface.OnClickListener() {
                    ...
                    public void onClick(DialogInterface dialog, int which) {
                        ...
                        SuaraData suaraData = new SuaraData(tpsList.getIdTps(), dprdprov.getId_calon(), Integer.parseInt(dprdprov.getSuara().trim()), "dprdprov");
                        sendsuaradata(suaraData, v, "dprdprov", dprdprov.getId_calon());
                    }
                });
